                ...
                <div class="row chat-section">
                    <div class="chat-cont custom-scrollbar col-sm-12 col-md-3 col-lg-3 col-xl-3">
                        <div v-if="my_user_chats.user_chats && my_user_chats.user_chats.length>0" class="account" v-cloak>
                            <button class="text-dark d-flex justify-content-between align-items-center w-100 pb-2" @click="selectAccount(0,true)" v-cloak>
                              <div style="width:90%">
                                  ...
                                  <span v-if="my_user_chats.all_mnr" class="mini-txt pl-2  text-left w-100">
                                      Hai @{{my_user_chats.all_mnr}} @{{my_user_chats.all_mnr<=1?'messaggio non letto':'messaggi non letti'}}
                        ...
                        v-show="page.page_chats!=''" class="account" v-cloak>
                            <button class="text-dark d-flex justify-content-between align-items-center w-100 pb-2" @click="selectAccount(index,false)">
                                <div style="width:90%">
                                    <span class="text-capitalize font-weight-bold text-left w-100 d-block pl-2">
                                        @{{page.name}}
                                    </span>
                                    <span v-if="page.all_mnr" class="mini-txt pl-2  text-left w-100">
                                        Hai @{{page.all_mnr}} @{{page.all_mnr<=1?'messaggio non letto':'messaggi non letti'}}
                                        <span class="notread"></span>
                                    </span>
                                </div>
                    ...
                    <div v-if="!is_mobile" class="col-sm-12 col-md-9 col-lg-9 col-xl-9 h-100" v-cloak>
                        <div v-if="displayed_name" :class="longtext?'chat-show-longtext chat-show':'chat-show'">
                            <div class="messages-cont custom-scrollbar" id="scroll-messages">
                                ...
                                <div v-if="in_load" class="d-flex justify-content-center align-items-center h-100" v-cloak>
                                    <div class="spinner-border text-secondary" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </div>
                                <div v-if="!in_load && messages.length==0" class="d-flex justify-content-center align-items-center h-100" v-cloak>
                                    <span>Ancora nessun messaggio</span>
                                </div>
                            </div>
                            <div class="messages-footer">
                                <textarea name="name" rows="1" v-model="message_text" type="text" name="" value="" class="custom-input-blue custom-scrollbar" placeholder=" {{__('Write a message')}}..."
                                id="mytextarea"
                                @input="text_wrap()"></textarea>
